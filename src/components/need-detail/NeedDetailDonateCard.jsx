...
function getDonationAmount(customAmount, selectedAmount) {
...
export default function NeedDetailDonateCard({ need }) {
	const [displayRaised, setDisplayRaised] = useState(need.raised);
	const progress = getNeedProgress({ ...need, raised: displayRaised });
	const donorCount = Math.max(24, Math.round(displayRaised / 229));
	const estimatedClose = getEstimatedClose(progress);
	const [animatedProgress, setAnimatedProgress] = useState(0);
	const [selectedAmount, setSelectedAmount] = useState(defaultDonationAmount);
	const [customAmount, setCustomAmount] = useState('');

	useEffect(() => {
		setDisplayRaised(need.raised);
	}, [need.id, need.raised]);
	...
	const activeDonationAmount = getDonationAmount(
		customAmount,
		selectedAmount,
	);

	const donateButtonLabel = useMemo(() => {
		return `Give ${formatCurrency(activeDonationAmount)}`;
	}, [activeDonationAmount]);

	function handleDonate() {
		setDisplayRaised(currentRaised => currentRaised + activeDonationAmount);
		setCustomAmount('');
	}
		...
		<aside className='lg:sticky lg:top-[105px] lg:self-start'>
			<div className='rounded-[18px] border border-hair bg-bone p-[30px]'>
				<div className='mb-2 font-fraunces text-5xl font-light italic leading-none tracking-[-0.02em] text-ink'>
					<span className='not-italic text-terra'>
				...
				<div className='mb-[14px] h-[5px] overflow-hidden rounded-full bg-hair'>
					<div
						className='h-full rounded-full bg-moss transition-[width] duration-1000 ease-out'
				...
				<div className='mb-2.5 text-[14.5px] font-medium text-ink'>
					Choose an amount
				</div>
				...
				<div className='mb-[14px] flex items-center overflow-hidden rounded-[10px] border border-hair bg-paper'>
					<span className='px-[14px] font-fraunces text-[19px] text-mute'>
						$
					</span>
					<input
						type='text'
